...
function softmax(x) {
    ...
    for(let i = 0; i < x.length; i++) {
        let max = Math.max.apply(null, x[i])
        let temp = x[i].map(v => Math.exp(v-max))
        let sum = 0
        for(let i = 0; i < temp.length; i++) {
            sum += temp[i]
        }
        arr.push(temp.map(v => v/sum))
    }
    return arr
}
...
function load_data2(N) {
    let min = 0
    let max = 10

    let x = Array(N).fill().map((v, i) => [min + (max-min)/N * i])
    let y = Array(N).fill().map(v => [Math.random() * (max-min)])
    y.sort((a, b) => a[0] - b[0])
    return [x, y]
}
...
Relu.prototype.forward = function(x) {
    let out = x.map(v => v.map(v2 => v2 > 0 ? v2 : 0))
    this.out = out
    return out
}
Relu.prototype.backward = function(dout) {
    return this.out.map((v, i) => v.map((v2, j) => v2 > 0 ? dout[i][j] : 0))
}
...
Softmax.prototype.backward = function(dout) {
    dx = self.out * dout
...
Adam.prototype.update = function(params, grads) {
    if(this.m == null) {
        ...
        for(param of params) {
            let zeros = null
            if(typeof param[0] == "object" && param[0].length != param[1].length) {
                zeros = []
                for(let i = 0; i < param.length; i++) {
                    zeros.push(this._zero(param[i]))
                }
            } else {
                zeros = this._zero(param)
            }
    ...
    this.iter += 1
    this.lr_t = this.lr * Math.sqrt(1.0 - Math.pow(this.beta2, this.iter)) / (1.0 - Math.pow(this.beta1, this.iter))

    for(let i = 0; i < params.length; i++) {
        if(typeof params[i][0] == "object" && params[i][0].length != params[i][1].length) {
            for(let j = 0; j < params[i].length; j++) {
                this._update(params[i][j], grads[i][j], this.m[i][j], this.v[i][j])
            }
        } else {
            this._update(params[i], grads[i], this.m[i], this.v[i])
        }
    }
}
...
Adam.prototype._update = function(p, g, m, v) {
    if(typeof p[0] == "number") {
        for(let j = 0; j < p.length; j++) {
            m[j] += (1-this.beta1) * (g[j] - m[j])
            v[j] += (1-this.beta2) * (Math.pow(g[j], 2) - v[j])
            p[j] -= (this.lr_t * m[j]) / (Math.sqrt(v[j]) + 1e-7)
        }
    } else {
        for(let j = 0; j < p.length; j++) {
            for(let k = 0; k < p[0].length; k++) {
                m[j][k] += (1-this.beta1) * (g[j][k] - m[j][k])
                v[j][k] += (1-this.beta2) * (Math.pow(g[j][k], 2) - v[j][k])
                p[j][k] -= this.lr_t * (m[j][k] / (Math.sqrt(v[j][k]) + 1e-7))
            }
        }
    }
}
...
Trainer.prototype.fit = function(x, t, max_epoch, batch_size, log_interval) {
    ...
    for(let epoch = 0; epoch < max_epoch; epoch++) {
        ...
        if(log_interval !== undefined && (epoch % log_interval == 0)) {
...
Trainer.prototype.test = function(x, t){
    let model = this.model
    console.log(model.forward(x, t))
}
...
let test = load_data(100)
trainer.test(test[0], test[1])